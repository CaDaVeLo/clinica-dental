...
function renderCalendario() {
    ...
    for (let d = 1; d <= dias_en_mes; d++) {
        ...
        const esPasado = new Date(anioActual, mesActual, d) < new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate());
        const seleccionado = fecha === fechaSeleccionada ? 'seleccionado' : '';
        const clase = esPasado ? '' : 'disponible';
 
        if (esPasado) {
            cal.innerHTML += `<span>${d}</span>`;
        } else {
            cal.innerHTML += `<span class="${clase} ${seleccionado}" onclick="seleccionarFecha('${fecha}', this)">${d}</span>`;
        }
    }
}
